...
function removeFirstOccurrences(str, value) {
  ...
  return (
    str.substring(0, indexOfValue) + str.substring(indexOfValue + value.length)
  );
}
...
function countVowels(str) {
  ...
  str
    ...
    .split('')
    .forEach((l) => {
      if (vowels.includes(l)) vowelsCount += 1;
    });
  return vowelsCount;
}
...
function isPalindrome(str) {
  const inputToCompare = str
    .toLowerCase()
    .split('')
    .filter((l) => l.charCodeAt(0) > 96 && l.charCodeAt(0) < 123)
    .join('');
  const palindromeToCompare = [...inputToCompare].reverse().join('');
  return inputToCompare === palindromeToCompare;
}
...
function invertCase(str) {
  return str
    .split('')
    .map((l) =>
      l.charCodeAt(0) > 96 && l.charCodeAt(0) < 123
        ? l.toUpperCase()
        : l.toLowerCase()
    )
    .join('');
...
function encodeToRot13(str) {
  return [...str]
    .map((l) => {
      if (
        (l.charCodeAt(0) >= 'a'.charCodeAt(0) &&
          l.charCodeAt(0) <= 'm'.charCodeAt(0)) ||
        (l.charCodeAt(0) >= 'A'.charCodeAt(0) &&
          l.charCodeAt(0) <= 'M'.charCodeAt(0))
      ) {
      ...
      if (
        (l.charCodeAt(0) >= 'n'.charCodeAt(0) &&
          l.charCodeAt(0) <= 'z'.charCodeAt(0)) ||
        (l.charCodeAt(0) >= 'N'.charCodeAt(0) &&
          l.charCodeAt(0) <= 'Z'.charCodeAt(0))
      ) {
        return String.fromCharCode(l.charCodeAt(0) - 13);
...
function getCardId(value) {
  ...
  const cardsSuitsArr = ['♣', '♦', '♥', '♠'];
  return (
    cardsArr.indexOf(value.slice(0, -1)) +
    cardsSuitsArr.indexOf(value[value.length - 1]) * cardsArr.length
  );
}
